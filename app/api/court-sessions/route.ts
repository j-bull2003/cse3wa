...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      ...
      return NextResponse.json(safeJson(one), { headers: cors })
    }
    const all = await prisma.courtSession.findMany({ orderBy: { createdAt: 'desc' } })
    return NextResponse.json(safeJson(all), { headers: cors })
  } catch (e) {
    console.error(e)
    return new NextResponse('Server error', { status: 500, headers: cors })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error(e)
    return new NextResponse('Invalid body', { status: 400, headers: cors })
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    return NextResponse.json(safeJson(updated), { headers: cors })
  } catch (e) {
    console.error(e)
    return new NextResponse('Invalid request', { status: 400, headers: cors })
  }
}
